...
int init_complex_filtergraph(FilterGraph *fg)
{
    ...
    /* 为AVFilterGraph 结构申请内存 */
    graph = avfilter_graph_alloc();
    if (!graph)
        return AVERROR(ENOMEM);

    /* 
        根据 graph_desc 秒速 初始化 graph inputs outputs 

        将一个字符串描述的图添加到一个图中
    */
    ret = avfilter_graph_parse2(graph, fg->graph_desc, &inputs, &outputs);
    av_log(NULL,AV_LOG_INFO,"[----- Test Filter ----] fg->graph_desc %s\n",fg->graph_desc);
    if (ret < 0)
        goto fail;

    for (cur = inputs; cur; cur = cur->next){
        av_log(NULL,AV_LOG_INFO,"-------> input_filter %s\n",cur->name);

        init_input_filter(fg, cur);
    }

    for (cur = outputs; cur;) {
        av_log(NULL,AV_LOG_INFO,"-------> output_filter %s\n",cur->name);
...
{
   // av_log(NULL,AV_LOG_INFO,"-------> configure_output_filter %s\n",out->name);
...
static int configure_input_filter(FilterGraph *fg, InputFilter *ifilter,
                                  AVFilterInOut *in)
{
   // av_log(NULL,AV_LOG_INFO,"-------> configure_input_filter %s\n",in->name);
