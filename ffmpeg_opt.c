...
static AVCodec *choose_decoder(OptionsContext *o, AVFormatContext *s, AVStream *st)
{
    ...
    if (codec_name) {
        ...
        return avcodec_find_decoder(st->codec->codec_id); /* 根据codec_id 查找编解码器 */
}
    ...
    */
    for (i = 0; i < ic->nb_streams; i++) {
        ...
        */
        ...
        /* 根据AVCodec 为 AVCodecContext 申请内存空间 */
        ...
        /* 将AVCodecContext 初始化 AVCodecContext */
        ...
        case AVMEDIA_TYPE_VIDEO:
            ...
            if (framerate 
                    && av_parse_video_rate(&ist->framerate,framerate) < 0) {
...
static int open_input_file(OptionsContext *o, const char *filename)
{
    ...
    /* 如果指定了格式名,根据格式名获取 AVInputFormat结构体 */
    if (o->format) {
    ...
    av_log(NULL,AV_LOG_INFO,"ic->nb_streams %d \n",ic->nb_streams);
    ...
    /*
        遍历AVFormatContext 中的所有数据流
        创建InputStream 结构并初始化 InputStream 结构部分变量
    */
...
static OutputStream *new_output_stream(OptionsContext *o, AVFormatContext *oc, enum AVMediaType type, int source_index)
{
    ...
    /* 根据avcodec(编码器)创建 AVCodecContext(编码器上下文)*/
    ...
    if (ost->enc) {
        ...
        if (preset 
                && (!(ret = get_preset_file_2(preset, ost->enc->name, &s)))) {
            do  {
                buf = get_line(s);
                if (!buf[0] || buf[0] == '#') {
                    av_free(buf);
                             ...
                             OutputStream *ost)
{
    ...
    /* 不允许同时设置 -filter 和 -filter_script 选项 */
    ...
    if (ost->filters_script)
        return read_file(ost->filters_script); /* 读取过滤器脚本文件 */
    else if (ost->filters)
        return av_strdup(ost->filters); /* 为过滤器字符串申请内存 */
...
static OutputStream *new_video_stream(OptionsContext *o, AVFormatContext *oc, int source_index)
{
    ...
    /* 
        创建新的输出流 
        创建 OutputStream 并初始化 OutputStream 结构部分变量
    */
    ...
    /* 检查 FilterGraph脚本的filter_script选项 */
...
static int init_complex_filters(void)
{
    ...
    /*
        初始化复杂过滤器图
    */
    av_log(NULL,AV_LOG_INFO,"[----- Test Filter ----] nb_filtergraphs %d\n",nb_filtergraphs);
...
static int open_output_file(OptionsContext *o, const char *filename)
{
    ...
    if (!strcmp(file_oformat->name, "ffm") 
        ...
        /* 发送文件到ffserver时的特殊情况:我们从ffserver获取流参数 */
...
/* 计算x在 OptionsContext 结构体中的相对偏移地址 */
