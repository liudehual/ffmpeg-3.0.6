                                     ...
                                     AVProbeData *pd)
{
    ...
    */
    ...
    /*
        探测成功，返回分数值(100 75 50 25 0 分数越高，格式匹配度越高)
    */
    if (fmt) {
        int i;
...
int ff_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for (;;) {
        ...
        */
        ...
        /*
        @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
            调用具体的 AVInputFormat 结构 回调函数，读取数据

            此处会进入具体的ffmpeg 模块
            <重要>
            如读取的是flv文件,则会进入 flv_read_packet函数
        @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        */
...
int av_read_frame(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (!genpts) {
        /* 
            读取数据包
            先检查数据包缓冲区是否有数据，有则直接获取，没有则先从协议(文件，网络等)
            读取到数据缓冲区中然后再从数据缓冲区获取数据
