...
int avformat_write_header(AVFormatContext *s, AVDictionary **options)
{
    ...
    if ((ret = init_muxer(s, options)) < 0)
        return ret;
    /*
        检查 write_header 回调函数是否为空，且不需要建立必要的比特流的过滤
        和提取任何额外的数据需要对于全局头。
        write_header、check_bitstream均为回调函数
    */
    if (s->oformat->write_header && !s->oformat->check_bitstream) {
        ...
        */
        ...
        /*
            将缓冲区内的数据刷新到指定协议中 
            avio_flush 函数最终会调用 AVIOContext 结构中的write_packet回调函数

            函数调用:avio_flush()-> flush_buffer() -> writeout()-> write_packet()
        */
...
static int write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if ((pkt->flags & AV_PKT_FLAG_UNCODED_FRAME)) {
        ...
        /*
            将数据通过指定 AVOutputFormat 结构的写入回调函数写入到文件中
        */
