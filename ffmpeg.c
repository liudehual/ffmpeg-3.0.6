...
static void sub2video_heartbeat(InputStream *ist, int64_t pts)
{
    ...
    /*
        当从文件中读取帧时，检查所有子视频流在同一文件中并再次发送子视频帧。
        否则，解码的视频帧可以积累在过滤器的图形，而过滤器（可能覆盖）绝望地等待一个
        字幕帧。
    */    
...
static void write_frame(AVFormatContext *s, AVPacket *pkt, OutputStream *ost)
{
    ...
    if (!ost->st->codec->extradata_size 
    ...
    */
    for (i = 0; i < nb_output_streams; i++) {
        ...
        while (1) {
            ...
            /* 
                获取过滤器帧数据并将数据放入到 filtered_frame 中
            */
...
static void flush_encoders(void)
{
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        for (;;) {
            ...
            case AVMEDIA_TYPE_AUDIO:
                encode = avcodec_encode_audio2; /* 调用指定音频编码模块的编码回调函数进行编码 */
                desc   = "audio";
                break;
            case AVMEDIA_TYPE_VIDEO:
                encode = avcodec_encode_video2; /* 调用指定视频编码模块的编码回调函数进行编码 */
                desc   = "video";
                break;
            default:
                stop_encoding = 1;
...
static int init_output_stream(OutputStream *ost, char *error, int error_len)
{
    ...
    if (ost->encoding_needed) {
        ...
        /* 检查码流参数，如果码流参数<1000,警告 */
        if (ost->enc_ctx->bit_rate && ost->enc_ctx->bit_rate < 1000)
            av_log(NULL, AV_LOG_WARNING, "The bitrate parameter is set too low."
                                         " It takes bits/s as argument, not kbits/s\n");

        /* 把输出流编码器上下文赋值给输出流的 AVStream codec指针 */
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->stream_copy) {
            ...
            */
            ...
            } else if(!(oc->oformat->flags & AVFMT_VARIABLE_FPS) /* 帧率可变，处理一下 */
            ...
            case AVMEDIA_TYPE_AUDIO:           /* 音频 */
                ...
                enc_ctx->channels           = dec_ctx->channels;             /* 音频通道数 */
                enc_ctx->frame_size         = dec_ctx->frame_size;           /* 音频帧中每个信道的样本数 */
                enc_ctx->audio_service_type = dec_ctx->audio_service_type;   /* 音频流传送的服务类型 */
                enc_ctx->block_align        = dec_ctx->block_align;          /* 音频包字节数 */
                enc_ctx->initial_padding    = dec_ctx->delay;                /* 播放延时 */
                enc_ctx->profile            = dec_ctx->profile;              /* 音频简介 */
            ...
            case AVMEDIA_TYPE_VIDEO:    /* 视频 */
                enc_ctx->pix_fmt            = dec_ctx->pix_fmt;              /* 视频像素格式(YUV或RGB等) */
                enc_ctx->width              = dec_ctx->width;                /* 视频宽 */
                enc_ctx->height             = dec_ctx->height;               /* 视频高 */
                enc_ctx->has_b_frames       = dec_ctx->has_b_frames;         /* 解码器中的帧重新排序缓冲区大小 */

                /* 设置采样率的纵横比 如果是未知的纵横比,则该值为0 */
                ...
                ost->st->avg_frame_rate = ist->st->avg_frame_rate;  /* 平均帧率 */
                ost->st->r_frame_rate = ist->st->r_frame_rate;      /* 实际帧率 */   
                break;
            case AVMEDIA_TYPE_SUBTITLE: /* 字幕 */
                enc_ctx->width  = dec_ctx->width;    /* 字幕宽 */
                enc_ctx->height = dec_ctx->height;   /* 字幕高 */
                break;
            case AVMEDIA_TYPE_UNKNOWN: /* 未知 */
            case AVMEDIA_TYPE_DATA:    /* 多媒体数据 */
            case AVMEDIA_TYPE_ATTACHMENT: /* 多媒体附属数据 */
                break;
            default:
                abort();    /* 异常 */
            }
...
static int need_output(void)
{
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->frame_number >= ost->max_frames) {
            ...
            /* 关闭当前AVFormatContext的所有输出流 */
            for (j = 0; j < of->ctx->nb_streams; j++)
                close_output_stream(output_streams[of->ost_index + j]);
            continue;
        }

        /* 既没有结束，也没有超出帧数量限制，接着输出 */
        return 1;
    }
...
#endif
    return av_read_frame(f->ctx, pkt); /* 获取一包数据 */
}
...
static int process_input(int file_index)
{
    ...
    */
    ...
    if(!ist->wrap_correction_done 
        ...
        /*
            校正基于功能流开始时间fixme这个理想前应先用做时间
            但我们不知道这是使流在的意思我们这里做为中断处理部分
        */
        if (   ist->next_dts == AV_NOPTS_VALUE
...
static int transcode_step(void)
{
    ...
    /*
        ...
        process_input() 
            ...
            -> decode_audio()/decode_video()/transcode_subtitles()
        
        decode_audio() 是调用 avcodec_decode_audio4() 来完成工作。
        decode_video() 是调用 avcodec_decode_video2() 来完成工作。
    */
    ...
    /*
        ...
        编码流程是：
        reap_filters() 
            -> do_video_out() -> avcodec_encode_video2() 或 
            -> do_audio_out() -> avcodec_encode_audio2()。
    */
 /*
  reap_filters
...
static int transcode(void)
{
    ...
    /*
        执行编码初始化
        编码器打开操作是在
        transcode_init() -> init_input_stream() -> avcodec_open2()完成的，
        对具体的编解码器进行初始化。例如X264_init()
    */
    ret = transcode_init();
    if (ret < 0)
        goto fail;
...
int main(int argc, char **argv)
{
    ...
    /*
        解析选项,打开输入、输出文件，创建过滤器链
        如果选项关联了回调函数则调用回调函数进行相应设置
    */
